...
# --- 2. NAVIGATION & MODE DEV (Placé ici pour éviter NameError) ---
...
def prepare_calendar_table(df_cal, df_history):
    ...
    for _, row in df_next.iterrows():
        ...
        if pred:
            ...
            if p_o < 0.6: del options["+2.5 Buts"] # Seuil confiance buts
...
def simulate_season_end(df_played, df_history, teams_list):
    # (Code inchangé pour la simulation de fin de saison)
    import itertools
    ...
    avg_h, avg_a = df_history['full_time_home_goals'].mean(), df_history['full_time_away_goals'].mean()
    team_forces = {}
    for t in teams_list:
...
selected_seasons = st.sidebar.multiselect("Historique (Poisson)", all_seasons, default=all_seasons) # Tout par défaut
...
if page == "Dashboard":
    ...
    # SCORECARDS (inchangé pour brevity, à conserver si souhaité)
    ...
    if not df_snap[df_snap['equipe'] == my_team].empty:
        ...
        c4.markdown(f'<div class="metric-card"><div class="metric-label">Diff.</div><div class="metric-value">{int(stats_team["total_diff"]):+d}</div></div>', unsafe_allow_html=True)
        ...
        if opp_sel == "Vue Globale":
            ...
            for o in teams:
                ...
                r = predict_match_advanced(df_hist, my_team, o) # Simulé domicile
            ...
            if res:
                with c_res:
                    ...
                    goal_txt = "+2.5 Buts" if res['over25'] > 0.6 else ("-2.5 Buts" if (1-res['over25']) > 0.6 else "Pas de pari buts sûr")
